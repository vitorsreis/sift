...
it('runs process jobs concurrently and preserves result order', function (): void {
    ...
    $waitForPeer = static fn(string $own, string $peer, string $output): string => sprintf(
        'file_put_contents(%s, "1"); $deadline = microtime(true) + 1; while (!file_exists(%s) && microtime(true) < $deadline) { usleep(10000); } echo file_exists(%s) ? %s : "missing";',
        var_export($own, true),
    ...
    $commands = [
        new PreparedCommand('first', PHP_BINARY, ['-r', $waitForPeer($firstMarker, $secondMarker, 'first')], getcwd() ?: '.', timeout: 2),
        new PreparedCommand('second', PHP_BINARY, ['-r', $waitForPeer($secondMarker, $firstMarker, 'second')], getcwd() ?: '.', timeout: 2),
    ];
