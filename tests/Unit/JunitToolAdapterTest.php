...
XML);
        ...
        expect($result->status)->toBe('failed')
            ...
            ->and($result->items[0]['message'])->toContain('Failed asserting that false is true.')
            ->and($result->items[0]['message'])->not->toContain('tests/FailingTest.php:16')
            ->and($result->items[0]['line'])->toBe(16)
            ->and($result->items[1]['type'])->toBe('skipped')
            ->and($result->items[1]['file'])->toBe('tests/SkippedTest.php')
            ->and($result->items[1]['message'])->toBe('Not applicable')
