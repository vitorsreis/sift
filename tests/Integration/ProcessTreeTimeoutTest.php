...
it('terminates child processes when a command times out', function (): void {
    ...
    $child = 'usleep(2000000); file_put_contents(' . var_export($marker, true) . ', "finished");';
    $parent = '$process = proc_open([' . var_export(PHP_BINARY, true) . ', "-r", ' . var_export($child, true) . '], [0 => ["pipe", "r"], 1 => ["pipe", "w"], 2 => ["pipe", "w"]], $pipes); usleep(3000000);';
